...
latam_1st_author_pubs<-all_georef %>% 
...
all_georef %>% 
  ...
  group_by(SO) %>% 
  tally() %>% 
  rename(n_latam_first=n)

total_pubs_jrnl<-left_join(total_pubs_jrnl,latam_pubs_jrnl,by="SO") %>% 
  mutate(percent_latam=n_latam_first/n*100)

```

### Publications

#### 1. No. of Publications _(all journals combined)_: N = `r comma(total_pubs)`   

#### 2. No. of Publications with at least 1 author from LatAm _(all journals combined)_: N = `r comma(latam_pubs_total)` (`r comma(latam_pubs_total/total_pubs*100)`%)  

#### 3. No. of Publications with 1st author from LatAm _(all journals combined)_: N = `r comma(latam_1st_author_pubs)` (`r comma(latam_1st_author_pubs/total_pubs*100)`%)  



--- 

#### 4. Publications per Journal (overall and with LatAm 1st Author)
...
top_countries %>% 
  ...
  rename(N=n) %>% 
  select("Country (ISO code)",N, percent) %>% 
   kbl() %>%
  # kable_styling() %>% 
  kable_styling(latex_options = "HOLD_position")
```
...
#### 3. In which region are authors based? 



```{r include = FALSE,echo = FALSE,message = FALSE,warning = FALSE}

top_regions<-all_georef %>% 
  # group_by(country.name) %>% 
  group_by(region) %>% 
  tally() %>% 
  drop_na() %>% 
...
#### 4. What are the most commonly represented affiliations? 

Top N = 20 Affiliations _(all journals combined)_ 

**_Preliminary Results_**: affiliations still require some cleaning

_Note: these are not unique authors (i.e., the same author at UF publishing 5 papers = UF x 5)_
...
#### 4. No. of coauthors & No. of countries on author list by region of 1st author 
